...
BEGIN
  ...
  IF v_user IS NULL AND NOT public.has_role(v_caller, 'admin') THEN
    RAISE EXCEPTION 'ensure_payment_summary: client is not linked to a portal account';
  END IF;

  IF v_caller IS NULL OR (NOT public.has_role(v_caller, 'admin') AND v_user IS DISTINCT FROM v_caller) THEN
    RAISE EXCEPTION 'ensure_payment_summary: access denied';
  END IF;

  INSERT INTO public.client_payment_summary (client_id, user_id)
  VALUES (p_client_id, v_user)
  ON CONFLICT (client_id) DO UPDATE
    SET user_id = COALESCE(public.client_payment_summary.user_id, EXCLUDED.user_id)

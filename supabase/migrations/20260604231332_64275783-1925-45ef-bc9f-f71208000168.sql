...
BEGIN
  ...
  -- Profile -> client.user_id backfill (only when email is unique on BOTH sides)
  ...
  -- Feature-table client_id backfill (only when user_id maps to exactly one client)
  WITH map AS (
    SELECT user_id, MIN(id) AS client_id, COUNT(*) AS n
    FROM public.clients
  ...
  dl AS (
    ...
    SET client_id = m.client_id
    FROM map m
    WHERE d.client_id IS NULL AND d.user_id::uuid = m.user_id AND NOT dry_run
    RETURNING d.id
  ),
  dl_count AS (
    SELECT COUNT(*) AS n FROM public.dispute_letters d
    JOIN map m ON m.user_id = d.user_id::uuid
  ...
  doc AS (
    ...
    SET client_id = m.client_id
    FROM map m
    WHERE dc.client_id IS NULL AND dc.user_id = m.user_id AND NOT dry_run
    RETURNING dc.id
  ),
  doc_count AS (
    SELECT COUNT(*) AS n FROM public.documents dc
    JOIN map m ON m.user_id = dc.user_id
  ...
  INTO v_payments_linked, v_uploads_linked, v_agreements_linked, v_disputes_linked, v_documents_linked;
